...
function initRoom(){
    ...
    var wallMaterial = new THREE.MeshMatcapMaterial();
    ...
    for(var j = 0; j < 4; j++){
      ...
      debugConsole.rotateY(THREE.Math.degToRad(-45-90*j));
      if (j == 0 || j == 1){
  ...
  new GUIVR.GuiVRButton("Height", 1.6, 0, 4, true, function(x){camera => camera.position.set(0,x,1);})
];
